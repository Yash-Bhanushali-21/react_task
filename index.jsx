...
const cityMapper = [
  ...
  const [colData, setColData] = useState([
    { field: "id", filter: "agNumberColumnFilter", maxWidth: "100",checkboxSelection:true,
    headerCheckboxSelection:true },
    { field: "name",cellStyle:(params) => (params.value===""?inValid:valid) },
    { field: "email" ,
    cellStyle:(params) => validate(params.value)
    
     },
    { field: "gender",  cellEditor:"agSelectCellEditor",
            cellEditorParams:{ values: gender,
            useFormatter : true
             } },
    { field: "country",cellEditor:"agSelectCellEditor",
            cellEditorParams:{ values: country_array,
            useFormatter : true
             } },
    { field: "city",cellEditor:"agSelectCellEditor",
    ...
    {field : "date", cellEditor:"datePicker",cellStyle:(params) => (params.value===""?inValid:valid) },
    {field : "DeleteAction" ,cellRendererFramework:(params)=> 
    <div>
    <i className="material-icons w3-large" 
    onClick={() => {
  ...
  const validate = (text) => {
    ...
    let first = new RegExp(/[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,15}/g).test(text);
    if(text.length <= 2 || !first ){
      return {borderColor:"yellow"};
    }

    //that means, a green flag for submission.
    setCanSubmit(true);
...
function createNewRowData() {
  ...
  const handleSubmit = () => {
   ...
   const updatedCol = colData.filter(word => word.field !== "DeleteAction");
    ...
    setOutputColData(updatedCol);
    setOutputRowData(rowOp);
  ...
  //keeping the submitted window as a JSX constant.
  const outputDisplay = <div id="myOutput" style={{width: '100%'}}className="ag-theme-alpine">
        <AgGridReact
          rowSelection = {'single'}
...
/*----event handler section close --- */
  return (
    <div style={{ width: '100%'}}>
      ...
      <button className="btn" onClick = {() => onRemoveSelected()}>Delete Selected</button>
      <button className="btn" onClick = {() => onRemoveDeselected()}>Delete Unselected</button>
      <button className="btn"  onClick={() => addRows()}>Add Row</button>
